...
class CustomDataset:
    ...
    def load_tracks(self):
        tracks = []
        for track_dir in os.listdir(self.root_dir):
            track_path = os.path.join(self.root_dir, track_dir)
            if os.path.isdir(track_path):
                track = self.load_track(track_path, track_dir)
                tracks.append(track)
        return tracks

    def load_track(self, track_path, track_name):
        audio_files = {source: os.path.join(track_path, source + ".wav") for source in ['vocals', 'bass', 'drums', 'piano', 'guitar', 'other', 'mixture']}
        audio_data = {source: self.load_audio(file) for source, file in audio_files.items()}
        return Track(track_name, audio_data)

    def load_audio(self, file_path):
        wav, sr = ta.load(file_path)
        return wav, sr
...
class Track:
    def __init__(self, name, audio_data):
        self.name = name
        self.audio = audio_data['mixture'][0].t().numpy()
        self.targets = {source: audio_data[source][0].t().numpy() for source in ['vocals', 'bass', 'drums', 'piano', 'guitar', 'other']}
...
def evaluate(solver, compute_sdr=False):
    ...
    test_set = CustomDataset("/raid/data/m13520010/demucs/mix_6_stems/test")
    ...
    with pool(args.test.workers) as pool:
        for index in indexes:
            ...
            references = th.stack(
                [th.from_numpy(track.targets[name]).t() for name in model.sources])
            if references.dim() == 2:
